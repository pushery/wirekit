...
@props([
    ...
    'iconOnly' => false,
    // The same idea on an axis: show the label where there is room for it and the glyph
    // alone where there is not. Takes a breakpoint NAME and measures the VIEWPORT.
    //
    // ⚠️ FIVE NAMES, NOT EIGHT, AND THE DIFFERENCE IS NOT COSMETIC. `table.th hideBelowContainer`
    // accepts eight because Tailwind's CONTAINER scale runs to `5xl`; the VIEWPORT scale stops at
    // `2xl`, which is why `table.th hideBelow` — the viewport half — accepts exactly these five.
    // Measured rather than copied: the first build here took the eight, and `max-3xl:` through
    // `max-5xl:` were emitted into the markup and generated by Tailwind NOWHERE. The drift guard
    // caught it; nothing at runtime would have, because an unknown class is valid HTML and the
    // button simply keeps its label.
    //
    // ⚠️ THE VIEWPORT, SAID OUT LOUD, BECAUSE THE SIBLING PROP MEASURES SOMETHING ELSE.
    // `table.th hideBelowContainer` measures a container the kit NAMES (`@container/wk-table`)
    // precisely so an `@`-variant cannot be silently retargeted by whatever a caller nested it
    // in. A button has no container of its own and sits anywhere, so a container-relative
    // variant here would have to measure "the nearest one, whichever that is" — which is the
    // ambiguity that naming was introduced to remove. A call site that really wants the
    // container question writes the two `@`-variants itself and knows which container it set.
    //
    // ⚠️ `iconOnly` WINS IF BOTH ARE GIVEN, and it says so rather than silently dropping one:
    // asking for "always the glyph" and "the glyph below lg" together is a contradiction, not
    // a refinement.
    'iconOnlyBelow' => null,
    'href' => null,
    'disabled' => false,
...
@php
    ...
    /*
     * The breakpoint name, against the same eight `table.th` publishes. `filled()` rather than
     * a null check: an unbound `icon-only-below=""` compiles to an empty string, which is a
    ...
    /*
     * Both at once is a contradiction rather than a refinement, and it is reported the same way
     * the missing-accessible-name check below reports its own: loud where it can be fixed.
     * `iconOnly` wins, because it is the stricter of the two and the one whose padding table
     * has already run.
     */
    if ($iconOnly && $iconOnlyBelow !== null) {
        $message = 'icon-only and icon-only-below contradict each other: the first hides the '
            .'label at every width, the second only below '.$iconOnlyBelow.'. icon-only wins; drop one.';
     ...
     */
    if (($iconOnly || $iconOnlyBelow !== null) && trim(strip_tags((string) $slot)) === '' && ! $attributes->has('aria-label') && ! $attributes->has('aria-labelledby')) {
    ...
    /*
     * The responsive half. Below the named width the button becomes the same square the static
     * `iconOnly` path produces; at and above it, it is an ordinary labeled button.
     *
     * ⚠️ THE SQUARE TRAVELS AS A CUSTOM PROPERTY RATHER THAN AS A CLASS, and that is not a
     * stylistic preference. Tailwind reads a literal class name, so an assembled one yields a
     * utility nothing generates — the rule the two tables above already state twice. Spelling
     * the width out per breakpoint AND per size would be eight prefixes times six sizes, which
     * is forty-eight literals for one prop. The prefix is the only part that has to be literal;
     * the token behind it is a value, so it travels inline and eight pairs cover every size.
     *
     * ⚠️ NO RESPONSIVE PADDING RESET SITS BESIDE THE WIDTH, AND ONE WAS TRIED. The static
     * `iconOnly` path replaces its padding table, so the obvious move was to zero the inline
     * padding below the breakpoint too — and taking it out again left the browser assertion
     * GREEN. `border-box` plus an explicit width fixes the outer box at the square whatever the
     * padding is, and the row's centering keeps the glyph in the middle of it; the padding only
     * shrinks a content box that nothing measures.
     *
     * ⚠️ AND THE UTILITY IS DELIBERATELY NOT SPELLED OUT ANYWHERE ABOVE, WHICH IS THE SECOND
     * HALF OF WHY IT IS GONE. Tailwind scans source TEXT, so a comment naming it regenerates it
     * — measured here: the first version of this note said the word, the bare rule reappeared in
     * the compiled stylesheet, and the reverse-drift scanner reported a selector no element
     * carries, emitted from this very paragraph. The reading bar's own anchor comment states
     * the same rule about itself.
     */
    $iconOnlySquareToken = match ($size) {
        'xs' => 'calc(var(--size-wk-sm)*0.875)',
        'sm' => 'var(--size-wk-sm)',
        'md-compact' => 'var(--size-wk-md-compact)',
        'lg' => 'var(--size-wk-lg)',
        'xl' => 'calc(var(--size-wk-lg)*1.1)',
    ...
    $iconOnlyBelowClasses = match ($iconOnlyBelow) {
        ...
        'xl' => 'max-xl:w-[var(--wk-button-square)]',
        '2xl' => 'max-2xl:w-[var(--wk-button-square)]',
        default => '',
    };

    /*
     * ⚠️ `sr-only` RATHER THAN `hidden`, AT BOTH ENDS, AND THAT IS THE ACCESSIBILITY HALF OF
     * THIS PROP. The label stays in the accessibility tree on every width: the name a screen
     * reader announces and the phrase voice control matches on do not depend on how wide the
     * window happens to be. `hidden` would make the button's name change when the reader
     * rotates their phone.
     *
    ...
    // Built here rather than inline so the custom property is emitted only when something
    // reads it. A style merge on every button would put a declaration nothing uses on all
    // of them.
    $buttonBag = $attributes->except('rel');
    if ($iconOnlyBelow !== null) {
        $buttonBag = $buttonBag->merge(['style' => '--wk-button-square: '.$iconOnlySquareToken.';']);
    }
...
>
    ...
    @elseif($iconOnlyBelow !== null)
        <span class="{{ $iconOnlyBelowLabelClass }}">{{ $slot }}</span>
    @else
        {{ $slot }}
    @endif
